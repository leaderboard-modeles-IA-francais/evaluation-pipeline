...
apptainer exec --no-mount home --nv --bind $PWD --bind ~/test-chat-templates.py --bind /var/lib/oar  --bind ~/.hf_token  --bind ~/.ssh --bind ~/clearml.conf ~/llm_benchmark_fr.sif bash -c '
